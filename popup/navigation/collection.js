function getNavItems() {
  ...
  const mainVisible = Boolean(mainContent && mainContent.style.display !== 'none');
  const settingsOpen = Boolean(settingsPanel && (settingsPanel.classList.contains('open') || settingsPanel.classList.contains('active')));
  const modalOpen = Boolean(modalOverlay && modalOverlay.classList.contains('open'));
  const conflictOpen = Boolean(conflictModalOverlay && conflictModalOverlay.classList.contains('open'));
